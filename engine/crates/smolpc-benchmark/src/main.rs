...
async fn main() -> Result<()> {
    ...
    match cli.command {
        Some(Command::Compare(args)) => compare::run_comparison(args),
        Some(Command::Run(args)) => run_benchmark(args).await,
        // Default to run if no subcommand — but requires --machine, so print help
        None => {
            // Try to parse as RunArgs for backward compat
...
async fn run_benchmark(cli: RunArgs) -> Result<()> {
    // --- Detect hardware ---
    ...
    // --- Compute prompt corpus hash ---
    let corpus_hash = prompt_corpus_hash();
    ...
    for (backend_idx, &backend) in config.backends.iter().enumerate() {
        ...
        for model_id in &config.models {
            ...
            let partial_report =
                build_report(&machine_info, &hw_snapshot, &config, &corpus_hash, &results, &skipped);
    ...
    // --- Final output ---
    let report = build_report(&machine_info, &hw_snapshot, &config, &corpus_hash, &results, &skipped);
...
fn build_report(
    ...
    BenchmarkReport {
        ...
        test_config: TestConfig {
            ...
            prompt_corpus_hash: Some(corpus_hash.to_string()),
        },
...
/// Compute a simple hash of the prompt corpus for comparison validity.
fn prompt_corpus_hash() -> String {
    ...
    for prompt in PROMPTS {
        ...
        prompt.content.hash(&mut hasher);
    }
