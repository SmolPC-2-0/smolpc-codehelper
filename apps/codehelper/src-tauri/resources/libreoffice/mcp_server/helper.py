...
if not HELPER_AUTH_TOKEN:
    logging.error("Missing %s in helper runtime environment", HELPER_AUTH_TOKEN_ENV)
    sys.exit(1)
...
def create_server_socket():
    ...
    server_socket.bind((HELPER_HOST, HELPER_PORT))
...
def handle_command(command):
    ...
    try:
        ...
        logging.error("Error handling command: %s", error)
        logging.error(traceback.format_exc())
        raise
...
def recv_command(client_socket):
    ...
    if len(length_data) != 4:
        raise HelperError("Incomplete request length header")

    msg_length = int.from_bytes(length_data, byteorder="big")
    if msg_length <= 0:
        raise HelperError("Invalid helper request frame length")
    if msg_length > MAX_HELPER_FRAME_SIZE:
        raise HelperError("Helper request exceeded the maximum frame size")

    payload = recv_exact(client_socket, msg_length)
    if len(payload) != msg_length:
        raise HelperError("Incomplete request payload")

    try:
        command = json.loads(payload.decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as error:
        raise HelperError(f"Invalid JSON received: {error}") from error

    if not isinstance(command, dict):
        raise HelperError("Helper request must be a JSON object")

    token = command.pop(HELPER_AUTH_TOKEN_FIELD, "")
    if not isinstance(token, str) or not hmac.compare_digest(token, HELPER_AUTH_TOKEN):
        raise HelperError("Unauthorized helper request")
...
def send_response(client_socket, response):
    ...
    if len(response_bytes) == 0:
        raise HelperError("Refusing to send an empty helper response")
    if len(response_bytes) > MAX_HELPER_FRAME_SIZE:
        raise HelperError("Helper response exceeded the maximum frame size")
...
logging.info("Starting command processing loop...")
try:
    while True:
        client_socket = None
        try:
            ...
            command = recv_command(client_socket)
            result = handle_command(command)
            response = {"status": "success", "result": result}
        except (socket.timeout, ConnectionResetError, ConnectionAbortedError) as error:
            logging.warning("Socket error while handling helper client: %s", error)
            response = None
        except HelperError as helper_error:
            logging.error("Helper error: %s", helper_error)
            response = {"status": "error", "error": str(helper_error)}
        except OSError as os_error:
            logging.error("OS error in helper server loop: %s", os_error)
            response = None
        except Exception as unexpected_error:
            logging.error("Unexpected helper server error: %s", unexpected_error)
            logging.error(traceback.format_exc())
            response = {"status": "error", "error": f"Unexpected error: {unexpected_error}"}

        if client_socket and response is not None:
            try:
                send_response(client_socket, response)
                logging.info("Response sent")
            except (ConnectionResetError, BrokenPipeError) as error:
                logging.warning("Client disconnected before response could be sent: %s", error)
            except Exception as send_error:
                logging.error("Failed to send response: %s", send_error)
        if client_socket:
            try:
                client_socket.close()
            except OSError as close_error:
                logging.error("Error closing client socket: %s", close_error)

except KeyboardInterrupt:
    logging.info("Helper server shutting down...")
except Exception as fatal_error:
    logging.fatal("Fatal helper server error: %s", fatal_error)
    logging.fatal(traceback.format_exc())
finally:
    try:
        server_socket.close()
        logging.info("Server socket closed")
    except OSError as final_close_error:
        logging.error("Error closing server socket: %s", final_close_error)
