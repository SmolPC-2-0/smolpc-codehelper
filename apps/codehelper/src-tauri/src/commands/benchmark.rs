...
pub async fn run_benchmark(
    ...
    Err(Error::Other(
        "Benchmarking is temporarily disabled while the benchmark path is migrated to the shared engine contract.".to_string(),
    ))
}
...
pub fn open_benchmarks_folder(app: tauri::AppHandle) -> Result<(), Error> {
    let dir = crate::benchmark::get_benchmarks_dir_with_app_handle(&app).map_err(|e| {
        Error::Other(format!(
            "Failed to locate benchmarks directory for opening: {e}"
        ))
    })?;
