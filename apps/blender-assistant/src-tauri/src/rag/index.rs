...
impl RagIndex {
    pub fn load_from_dir(rag_dir: &Path) -> Self {
        ...
        let metadata = match load_metadata(&metadata_json_path) {
...
fn load_metadata(json_path: &Path) -> Result<Vec<RagChunk>, String> {
    if json_path.exists() {
    ...
    Err(format!("No metadata file found at {}", json_path.display()))
}
