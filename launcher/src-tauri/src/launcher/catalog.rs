use super::types::{LauncherManifest, LauncherManifestApp};
...
fn validate_manifest(manifest: &LauncherManifest) -> Result<(), String> {
    ...
    for app in &manifest.apps {
        ...
        validate_launch_command(app_id, app.launch_command.as_ref())?;
...
fn validate_launch_command(
    ...
    if launch_command.is_empty() {
        return Err(format!(
            "Launcher app '{app_id}' launch_command must contain at least one token"
        ));
    }
    if launch_command[0].trim().is_empty() {
        return Err(format!(
            "Launcher app '{app_id}' launch_command executable token cannot be empty"
        ));
    }
