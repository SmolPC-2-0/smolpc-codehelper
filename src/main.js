...
function setupEventListeners() {
    ...
    document.getElementById('newChatBtnHeader').addEventListener('click', createNewChat);
    
    // Context toggle
    document.getElementById('contextToggle').addEventListener('click', toggleContext);
...
function renderChatList() {
    ...
    if (chats.length === 0) {
        chatList.innerHTML = '<div style="padding: 20px; text-align: center; color: #94a3b8;">No chats yet. Start a new conversation!</div>';
        return;
    }
...
function renameChat(chatId) {
    ...
    const titleElement = document.querySelector(`.chat-item-title[data-chat-id="${chatId}"]`);
    const currentTitle = chat.title;
    
    titleElement.classList.add('editing');
    titleElement.innerHTML = `<input type="text" class="chat-item-input" value="${escapeHtml(currentTitle)}" />`;
...
function toggleContext() {
    ...
    const toggle = document.getElementById('contextToggle').querySelector('.toggle-switch');
    const indicator = document.getElementById('contextIndicator');
    
    if (contextEnabled) {
        toggle.classList.add('active');
        indicator.classList.remove('off');
        indicator.querySelector('.indicator-text').textContent = 'Context ON - I remember this conversation';
        indicator.querySelector('.indicator-icon').textContent = '💡';
    } else {
        toggle.classList.remove('active');
        indicator.classList.add('off');
        indicator.querySelector('.indicator-text').textContent = 'Context OFF - Each question is fresh';
        indicator.querySelector('.indicator-icon').textContent = '⚡';
    }
...
function loadChatsFromStorage() {
    ...
    try {
        ...
        chats = data.chats || [];
        contextEnabled = data.contextEnabled !== undefined ? data.contextEnabled : true;
