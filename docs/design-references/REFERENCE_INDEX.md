# Design References Index

Last Updated: 2026-02-10

## User-Provided References

1. `docs/design-references/Reference 1.png`
2. `docs/design-references/Reference 2.png`
3. `docs/design-references/Reference 3.png`

## Round 1 Kept Online References

1. #1 Raycast AI Chat
   - Link: https://www.raycast.com/changelog/macos/1-69-0
   - Capture: `docs/design-references/kept-online/01-raycast-new-ai-chat.png`

2. #3 ChatGPT Desktop
   - Link: https://openai.com/chatgpt/desktop/
   - Capture: `docs/design-references/kept-online/03-openai-chatgpt-desktop.png`
   - Note: capture is blocked by Cloudflare challenge in headless mode.

3. #4 Dribbble (AI Chat Interface in Light Theme)
   - Link: https://dribbble.com/shots/24101037-AI-Chat-Interface-in-Light-Theme
   - Capture: `docs/design-references/kept-online/04-dribbble-light-theme-ai-chat.png`

4. #5 Dribbble (AI Chat Interface)
   - Link: https://dribbble.com/shots/26883616-AI-Chat-Interface
   - Capture: `docs/design-references/kept-online/05-dribbble-ai-chat-interface.png`

5. #7 Dribbble (AI chat assistant model)
   - Link: https://dribbble.com/shots/25220634-AI-chat-assistant-model
   - Capture: `docs/design-references/kept-online/07-dribbble-ai-chat-assistant-model.png`

6. #8 Dribbble (Light vs Dark Sidebar)
   - Link: https://dribbble.com/shots/22501121-Light-vs-Dark-Sidebar
   - Capture: `docs/design-references/kept-online/08-dribbble-light-vs-dark-sidebar.png`

7. #9 Linear
   - Link: https://linear.app/
   - Capture: `docs/design-references/kept-online/09-linear-homepage.png`

## Round 2 Candidate Set (Decided)

1. R2-01 Linear new UI changelog
   - Link: https://linear.app/changelog/2024-03-20-new-linear-ui
   - Capture: `docs/design-references/round-2-candidates/r2-01-linear-new-ui-changelog.png`
   - Decision: kept

2. R2-02 Linear redesign deep dive
   - Link: https://linear.app/now/how-we-redesigned-the-linear-ui
   - Capture: `docs/design-references/round-2-candidates/r2-02-linear-redesign-deep-dive.png`
   - Decision: kept

3. R2-03 Notion 2.39
   - Link: https://www.notion.com/releases/2024-04-30
   - Capture: `docs/design-references/round-2-candidates/r2-03-notion-239-sidebar-basics.png`
   - Decision: rejected (boring despite good functionality)

4. R2-04 Notion 2.40
   - Link: https://www.notion.com/en-gb/releases/2024-06-11
   - Capture: `docs/design-references/round-2-candidates/r2-04-notion-240-sidebar-organize.png`
   - Decision: rejected (boring despite good functionality)

5. R2-05 GitHub Copilot Chat GA
   - Link: https://github.blog/changelog/2025-07-09-new-copilot-chat-features-now-generally-available-on-github
   - Capture: `docs/design-references/round-2-candidates/r2-05-github-copilot-chat-ga.png`
   - Decision: kept (button/widget style)

6. R2-06 Behance: Sidebar for AI chats
   - Link: https://www.behance.net/gallery/243521617/Sidebar-for-AI-chats
   - Capture: `docs/design-references/round-2-candidates/r2-06-behance-sidebar-for-ai-chats.png`
   - Decision: kept

## Archived Exploratory Candidates (Lower Fit)

These were gathered during exploration and archived to avoid cluttering active review:

- `docs/design-references/archive-ignored/r2-01-linear-redesign-article.png`
- `docs/design-references/archive-ignored/r2-02-github-copilot-chat-ga.png`
- `docs/design-references/archive-ignored/r2-03-notion-qna.png`
- `docs/design-references/archive-ignored/r2-04-claude-artifacts-help.png`
- `docs/design-references/archive-ignored/r2-05-dribbble-ai-assistant-dashboard.png`
- `docs/design-references/archive-ignored/r2-06-perplexity-app-store.png`

## Rejected Set (for context)

1. #2 Raycast Chat Branching
   - Link: https://www.raycast.com/changelog/1-101-0
   - Reason: branch feature out of scope.

2. #6 Dribbble popup window
   - Link: https://dribbble.com/shots/25974089-AI-Chat-popup-window-for-agents-with-3-suggestions
   - Reason: style perceived as cheap.
